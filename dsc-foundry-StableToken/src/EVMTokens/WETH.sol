...
contract WETH is BurnMintERC20, ReentrancyGuard {
    // -------------Custom Errors----------------------
    error InvalidUser();
    ...
    error InsufficientWETHBalance(uint256 available, uint256 requested);
    error ZeroAmount();
    ...
    constructor() BurnMintERC20("Wrapped Ethereum", "WETH", 18, 0, 0) {}
    ...
    /* -----------External Functions------------ */
    function depositEthAndMintWETH() public payable {
    ...
    function withdrawEthAndBurnWETH(uint256 amountToBurn) external nonReentrant {
        if (amountToBurn == 0) revert ZeroAmount();
        if (balanceOf(msg.sender) < amountToBurn) revert InsufficientWETHBalance(balanceOf(msg.sender), amountToBurn);
