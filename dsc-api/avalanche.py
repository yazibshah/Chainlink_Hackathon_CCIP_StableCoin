...
# Helper function to check if a contract function exists
def function_exists(contract, function_name: str) -> bool:
    try:
        return hasattr(contract.functions, function_name)
    except Exception:
        return False
...
if not w3.is_connected():
    raise HTTPException(status_code=500, detail="Failed to connect to the Avalanche Fuji node")
...
# Validate DSCEngine contract deployment
if not is_contract_deployed(w3, contract_address):
    raise RuntimeError(f"No contract deployed at {contract_address} on Avalanche Fuji")

# Initialize contracts
contract = w3.eth.contract(address=contract_address, abi=abi)
if not function_exists(contract, "getCollateralTokens"):
    raise RuntimeError("getCollateralTokens function not found in DSCEngine contract ABI")
...
async def get_collateral_tokens():
    try:
        ...
        if not tokens:
            return {"collateral_tokens": [], "message": "No collateral tokens configured in DSCEngine contract"}
        ...
        raise HTTPException(status_code=500, detail=f"Invalid contract call: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch collateral tokens: {str(e)}")
