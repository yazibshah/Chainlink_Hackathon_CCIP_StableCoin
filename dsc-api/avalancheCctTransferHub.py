...
class SendAvaxRequest(BaseModel):
    amount: float
...
# Web3 setup
AVAX_RPC_URL = os.getenv("AVALANCHE_RPC_URL")
PRIVATE_KEY = os.getenv("PRIVATE_KEY")
DSC_TRANSFER_HUB_ADDRESS = os.getenv("AVALANCHE_DSC_TRANSFER_HUB_ADDRESS")

# Validate environment variables
required_vars = {
    "AVALANCHE_RPC_URL": AVAX_RPC_URL,
    "PRIVATE_KEY": PRIVATE_KEY,
    "AVALANCHE_DSC_TRANSFER_HUB_ADDRESS": DSC_TRANSFER_HUB_ADDRESS
}
...
if not is_valid_address(DSC_TRANSFER_HUB_ADDRESS):
    raise RuntimeError(f"Invalid Avalanche address for DSC_TRANSFER_HUB_ADDRESS: {DSC_TRANSFER_HUB_ADDRESS}")
...
# Verify chain ID (Avalanche Fuji = 43113)
if w3.eth.chain_id != 43113:
    raise RuntimeError(f"Connected to wrong network. Expected chain ID 43113 (Avalanche Fuji), got {w3.eth.chain_id}")

if not w3.is_connected():
    raise HTTPException(status_code=500, detail="Failed to connect to Avalanche node")
...
if not is_contract_deployed(w3, dsc_transfer_hub_address):
    raise RuntimeError(f"No contract deployed at {dsc_transfer_hub_address} on Avalanche Fuji")
...
@app.post("/send-avax")
async def send_avax(data: SendAvaxRequest):
    """Send AVAX to DSCTransferHub to hit the receive function"""
    try:
        if data.amount <= 0:
            raise HTTPException(status_code=400, detail="Amount must be positive")
        
        amount_in_wei = w3.to_wei(data.amount, "ether")
        gas_limit = 100000  # Match Sepolia's send-eth gas limit
        ...
        # Verify contract can receive AVAX
        if not is_contract_deployed(w3, dsc_transfer_hub_address):
            raise HTTPException(status_code=500, detail=f"No contract deployed at {dsc_transfer_hub_address}")

        # Send AVAX directly to DSCTransferHub contract to trigger receive()
        ...
        # Check AVAX deposits after transaction
        avax_deposits = dsc_transfer_hub.functions.userEthDeposits(public_address).call()
        avax_balance_after = w3.eth.get_balance(public_address)
        
        # Verify AVAX deposit increase
        if avax_deposits < amount_in_wei:
            raise HTTPException(
                status_code=500,
                detail=f"AVAX not deposited: expected increase of {w3.from_wei(amount_in_wei, 'ether')} AVAX, got {w3.from_wei(avax_deposits, 'ether')} AVAX in deposits"
            )
        ...
        return {
            ...
            "avax_deposits": w3.from_wei(avax_deposits, "ether"),
            "events": events,
            "tx_data": "0x"
        }
    except ContractLogicError as e:
        ...
        raise HTTPException(status_code=500, detail=f"AVAX transfer failed: {str(e)}")

@app.post("/transfer-tokens")
async def transfer_tokens(data: TransferTokensRequest):
    """Transfer tokens cross-chain to Sepolia"""
    try:
        ...
        amount_in_wei = w3.to_wei(data.amount, "ether")
        destination_chain_selector = 16015286601757825753  # Sepolia (unchanged)
        ...
        # Check sufficient AVAX for fees
        avax_deposit = dsc_transfer_hub.functions.userEthDeposits(public_address).call()
        if avax_deposit == 0:
            raise HTTPException(status_code=400, detail="Insufficient AVAX deposit for fees")
...
async def withdraw_deposits(data: WithdrawDepositsRequest):
    """Withdraw remaining tokens and AVAX from DSCTransferHub"""
    try:
        ...
        avax_balance = dsc_transfer_hub.functions.userEthDeposits(public_address).call()
        
        if token_balance == 0 and avax_balance == 0:
